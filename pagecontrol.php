            ...
            $tabval = '<div class="tab-pane fade container show active" id="'.$row["vacancy_id"].'" role="tabpanel">
                    <div class="row">
                        ...
                        <div class="col-md-3 col-sm ">
                            <a href="account/account_confirm.php?id='.$row["vacancy_id"].'"><button class="btn btn-sm btn-outline-info float-right" id="show_job">Apply Now</button></a>
                        </div>
                    ...
                    <div class="row justify-content-end">
                        <a href="account/account_confirm.php?id='.$row["vacancy_id"].'"><button class="btn btn-sm btn-outline-info my-3 pull-right" id="show_job">Apply Now</button></a>
                    </div>                  
                <hr>
            </div>';
        }                
        ...
        foreach($cats as $jrow){
            ...
            if($result->rowCount() > 0){
                foreach($posts as $prow){
                    $catdisplay[] = '<a class="nav-link links text-muted" id="'.$prow["vacancy_id"].'" data-toggle="pill" href="#'.$prow["vacancy_id"].'" role="tab">'.$prow["emptype"].'</a>'; 
                }
            } elseif(rowCount() < 0){
